...
当前版本：**内测 v2.0.0**（详见 [`doc_auto/内测-v2.0-实施方案.md`](doc_auto/内测-v2.0-实施方案.md) 与 [`doc_auto/changes.md`](doc_auto/changes.md)）

> v2.0（变体 A）交付**对话式规划**：点击首页「🗨 对话式规划」，用大白话多轮描述行程，助手抽取硬约束（目的地/景点/天数/策略/出行/体力/酒店）、必填缺失时主动澄清、结构化确认后**自动回填左侧表单并复用现有 `plan/replan` 直接出路书**。新增对话维度内存埋点（`dialog_turn`/`constraint_extract`/`clarify`/`dialog_refine`/`evidence_ref`，schema 升至 2.0.0）。**跨会话记忆 / 对话回放**因当前部署（Render 临时文件系统）无持久盘而 **PENDING**（见实施方案 §0），待持久化存储方案落地后解锁。
>
> v1.7 ACTIVE 子集仍保留：`request_summary` / `token_usage` 内存埋点 + 单次指标计算（`metrics.js`）+ 评测集单次跑通（`eval/`）。
...
- **可观测与评测 ACTIVE 子集（v1.7）**：新增 `token_usage`（每次 LLM 调用累计 token）与 `request_summary`（请求级汇总：总耗时/最终状态/修复轮次/token）两类内存埋点，trace schema 升至 1.7.0；`metrics.js` 从单次/内存 trace 计算核心指标（成功率、违例数、修复轮次、延迟、缓存命中率、局部重算复用率、token 等）并支持批量即时聚合；`eval/`（评测集 + 运行器）可对标准场景单次跑通并出当次报告（NON-GATING，不留 baseline）。落盘/回放/门禁 PENDING（见 §0）
